							...
							<?php if(class_exists("WpFastestCachePowerfulHtml")){ ?>
								<?php if(method_exists("WpFastestCachePowerfulHtml", "render_blocking")){ ?>
									...
									if(in_array(get_bloginfo('language'), $tester_arr) || in_array(str_replace("www.", "", $_SERVER["HTTP_HOST"]), $tester_arr)){ ?>
										<div class="questionCon">
											<div class="question">Render Blocking</div>
											<div class="inputCon"><input type="checkbox" <?php echo $wpFastestCacheRenderBlocking; ?> id="wpFastestCacheRenderBlocking" name="wpFastestCacheRenderBlocking"><label for="wpFastestCacheRenderBlocking">Eliminate render-blocking JavaScript and CSS in above-the-fold content</label> <b style="color:red;">(BETA)</b></div>
										</div>
									<?php } ?>
								<?php } ?>
							<?php } ?>
