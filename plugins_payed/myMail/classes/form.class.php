...
class mymail_form {
	...
	private function get_hidden_fields() {
		...
		$redirect = home_url(remove_query_arg(array('mymail_error', 'mymail_success'), esc_url($_SERVER['REQUEST_URI'])));
		...
		//if($this->extern) $html .= '<input name="_extern" type="hidden" value="1">'."\n";
		$html .= '<input name="formid" type="hidden" value="'.$this->ID.'">'."\n";

		//echo '<textarea style="width:100%;height:200px">'.print_r(htmlspecialchars($html), true).'</textarea>';
		return $html;
	}
	...
	public function unsubscribe( ) {
		...
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
			...
			$return['html'] = $return['success']
				...
				: (empty($_POST['email'])
					? __('Please enter your email address', 'mymail')
			...
			if($return['success']){
				...
				wp_die($return['html'].'<a href="javascript:history.back()">'.(empty($_POST['email']) ? __('Please enter your email address', 'mymail')	: mymail_text('unsubscribeerror')).'</a>');
			}
			exit;
